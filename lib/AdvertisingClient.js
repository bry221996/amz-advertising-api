...
class AdvertisingClient {
  ...
  async getSnapshot(campaignType, snapshotId) {
    if (!snapshotId) {
      throw new Error('No valid snapshotId');
    }
    ...
    while (true) {
      ...
      if (snapshotRequest.status === 'SUCCESS') {
        ...
        throw new Error('SNAPSHOT GENERATION FAILURE');
      } else if (snapshotRequest.status === 'ERROR') {
        throw new Error('SNAPSHOT GENERATION ERROR');
      }
  ...
  createReport(data) {
    return this.apiRequest('/reporting/reports', data, `POST`);
  }

  async getReportV3(reportId, raw = false) {
    let retry = 1;

    while (true) {
      let reportRequest = await this.apiRequest(`reporting/reports/${reportId}`, null, 'GET');

      if (reportRequest.status === 'COMPLETED') {
        var result = await this.download(reportRequest.url, false, 1, raw);

        if (raw) return result;

        return Array.isArray(result) ? result : JSONbig.parse(zlib.gunzipSync(result).toString());
      } else if (reportRequest.status === 'FAILURE') {
        throw new Error('REPORT GENERATION FAILURE');
  ...
  async getReport(reportId, raw = false) {
    if (!reportId) {
      throw new Error('No valid reportId');
    }
  ...
  async download(location, auth, retry = 1, raw = false) {
    let requestOptions = {
      accept: '*',
      headers: {
    ...
    if (requestFailed || response.statusCode == '429' || response.statusCode == '500' || response.statusCode == '401') {
      ...
      await sleep(waitTime);
      return this.download(location, true, ++retry, raw);
    } else if (response.statusCode == 307) {
      return this.download(response.headers.location, false, 1, raw);
