...
const endpointOf = require('./endpoints');
...
class AdvertisingClient {
  ...
  listProfiles() {
    return this.request('GET', endpointOf('profiles'));
  }

  getProfile(profileId) {
    return this.request('GET', `${endpointOf('profiles')}/${profileId}`);
  }

  listPortfolios(data = {}, extended = false) {
    return this.request('GET', `${endpointOf('portfolios', extended)}${queryfy(data)}`);
  }

  getPortfolio(portfolioId, extended = false) {
    return this.request('GET', `${endpointOf('portfolios', extended)}/${portfolioId}`);
  }
