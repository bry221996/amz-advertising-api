...
class AdvertisingClient {
  ...
  listCampaignNegativeKeywords(data = {}, extended = false) {
    return this.request('GET', `${endpointOf('sponsoredProducts', 'campaignNegativeKeywords', extended)}${queryfy(data)}`);
  }

  getCampaignNegativeKeyword(keywordId, extended = false) {
    return this.request('GET', `${endpointOf('sponsoredProducts', 'campaignNegativeKeywords', extended)}/${keywordId}`);
  }
